...
const FavoritePage = () => {
  ...
  const movies = useMemo(() => {
    ...
    let filteredMovies = favorites.filter(
      (favorite) => favorite.media_type === "movie",
    );
  ...
  }, [favorites, mediaType, genre]);

  const tvs = useMemo(() => {
    if (!favorites || !favorites.length) return [];
    let filteredTvs = favorites.filter(
      (favorite) => favorite.media_type === "tv",
    );
  ...
  const Layout = ({ children }: Readonly<{ children: React.ReactNode }>) => (
    <section className="mx-auto w-full max-w-7xl px-5 pt-25 md:px-10">
      <Card className="rounded-none">
        <h2 className="mb-5 text-xl font-semibold md:text-2xl">MY FAVORITES</h2>
