...
const RatingPage = () => {
  ...
  const movies = useMemo(() => {
    ...
    let filteredMovies = ratings.filter(
      (rating) => rating.media_type === "movie",
    );
  ...
  }, [ratings, mediaType, genre]);

  const tvs = useMemo(() => {
    if (!ratings || !ratings.length) return [];
    let filteredTvs = ratings.filter((rating) => rating.media_type === "tv");
  ...
  const Layout = ({ children }: Readonly<{ children: React.ReactNode }>) => (
    <section className="mx-auto w-full max-w-7xl px-5 pt-25 md:px-10">
      <Card className="rounded-none">
        <h2 className="mb-5 text-xl font-semibold md:text-2xl">MY RATINGS</h2>
...
>
  <TabsList className="h-fit gap-5 bg-transparent p-0">
    ...
    >
      Movie{" "}
      <span className="font-medium text-neutral-300">{movies.length}</span>
    </TabsTrigger>
    <TabsTrigger
      value="tv"
      className="data-[state=active]:border-b-primary data-[state=active]:bg-accent-foreground h-fit cursor-pointer rounded-none border-b-2 p-0 font-semibold transition-all duration-300 data-[state=active]:scale-105"
    >
      TV Shows{" "}
      <span className="font-medium text-neutral-300">{tvs.length}</span>
    </TabsTrigger>
  </TabsList>
</Tabs>; */
}
